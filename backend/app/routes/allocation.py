...
async def auto_fill_sheet(
    ...
    **Returns:**
    ...
    try:
        ...
        # Count updated cells
        updated_count_row = await fetch_one("""
            SELECT COUNT(*) as count
            FROM allocation_sheet_cells
            WHERE sheet_id = $1
              AND sent_quantity > 0
        """, sheet_id)
        updated_count = updated_count_row['count'] if updated_count_row else 0
        
        # Get shortfalls
        shortfalls = await fetch_all("""
            SELECT
                c.item_id,
                i.name as item_name,
                c.customer_id,
