...
function ZohoItemMaster() {
        ...
        try {
            ...
            if (response.items && response.items.length > 0) {
                const syncTimes = response.items
                    .map((item: ZohoItem) => item.last_sync_at)
    ...
    // Handle item update
    const handleItemUpdate = async (updatedRow: ZohoItem, originalRow: ZohoItem) => {
        try {
            const changes: Partial<ZohoItem> = {};

            // Find changed fields
            (Object.keys(updatedRow) as Array<keyof ZohoItem>).forEach((key) => {
                // @ts-ignore
                                    ...
                                    {syncProgress && (
                                        <>
                                            <Typography variant="h6" sx={{ mt: 2, mb: 1 }}>
                                                {syncProgress.current} / {syncProgress.total} items ({syncProgress.percentage}%)
                                            </Typography>
