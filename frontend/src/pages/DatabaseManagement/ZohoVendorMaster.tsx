...
function ZohoVendorMaster() {
        ...
        try {
            ...
            if (response.vendors && response.vendors.length > 0) {
                const syncTimes = response.vendors
                    .map((item: ZohoVendor) => item.last_sync_at)
    ...
    // Handle item update
    const handleItemUpdate = async (updatedRow: ZohoVendor, originalRow: ZohoVendor) => {
        try {
            const changes: Partial<ZohoVendor> = {};

            // Find changed fields
            (Object.keys(updatedRow) as Array<keyof ZohoVendor>).forEach((key) => {
                // @ts-ignore
                                    ...
                                    {syncProgress && (
                                        <>
                                            <Typography variant="h6" sx={{ mt: 2, mb: 1 }}>
                                                {syncProgress.current} / {syncProgress.total} vendors ({syncProgress.percentage}%)
                                            </Typography>
