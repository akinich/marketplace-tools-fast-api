...
const SalesOrderForm: React.FC = () => {
    ...
    // Load initial data
    useEffect(() => {
        // Load items (keep items bulk load for now or optimize later, usually items are fewer or freq used)
...
const loadOrderData = async () => {
    if (!id) return;
    // ... existing loadOrderData logic needed ...
    setLoading(true);
    try {
        ...
        if (order.customer_id) {
            try {
                // We might need a getCustomerById to ensure it's in the list for Autocomplete
            ...
            } catch (e) { console.error("Could not load order customer details", e); }
        }
